-- statistiche_richieste calcola 'stati_trattativa' e 'assegnatari' solo a
-- partire da `r` (le richieste dal sito nel periodo, form_contatti): una
-- trattativa aperta in agenda o al banco (walk-in) non ha mai una riga
-- form_contatti, quindi restava fuori da questi due grafici anche se già
-- vinta — mentre 'trattative_aperte'/'vinte'/'perse' qui sotto la contavano
-- già bene, interrogando opportunita direttamente.
--
-- La correzione aggiunge, solo a questi due campi, le trattative senza
-- nessuna richiesta collegata (anti-join su form_contatti, lo stesso usato
-- per l'audit) che sono nel periodo per data di apertura o di chiusura — le
-- stesse due date che 'trattative_aperte' e 'trattative_vinte'/'perse' già
-- usano per includerle nei totali.
--
-- Da eseguire nel SQL Editor del progetto Supabase Ronchiverdi
-- (upoiasekisojikbzsymq), dopo 2026-09-16-trattativa-di-agenda-tracciata.sql
-- (che dà origine = 'agenda' alle trattative di agenda già esistenti: non è
-- un requisito di questa funzione, che riconosce le trattative senza
-- richiesta con l'anti-join e non guardando `origine`, ma tiene i due dati
-- coerenti).
...
AS $function$
	...
	select jsonb_build_object(
		...
		-- Coppie sorgente/mezzo grezze: l'app le classifica in canali di
		-- traffico con la stessa logica di TCA.
		'coppie_utm', coalesce((
			select jsonb_agg(to_jsonb(x))
			from (
				select utm_source, utm_medium, referrer, gclid, fbclid, count(*) as richieste
