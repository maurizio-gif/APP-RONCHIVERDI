-- Un abbonamento che non è da rinnovare né da trattare (es. un caso fuori
-- scope, un errore di prodotto, un socio già gestito altrove) va tolto dal
-- conteggio di chi lavora i rinnovi, senza sparire dai dati: resta in
-- tabella e nel grafico, come terza fetta a sé — non un rinnovato, non un
-- non-rinnovato, verificabile in ogni momento da chi vuole controllare cosa
-- è stato escluso e perché. Manuale come stato_manuale e nota: nessuna
-- sincronizzazione lo scrive.
--
-- Da eseguire nel SQL Editor del progetto Supabase Ronchiverdi
-- (upoiasekisojikbzsymq), dopo 2026-09-25-abbonamenti-scadenze-motivo-non-rinnovo.sql.

alter table public.abbonamenti_scadenze_lavorazione
	add column if not exists escluso_da_report boolean not null default false;

comment on column public.abbonamenti_scadenze_lavorazione.escluso_da_report is
	'Vero se chi lavora la scadenza l''ha segnata come fuori dal conteggio di rinnovi/trattative (es. caso non pertinente) — resta comunque visibile e consultabile. Manuale: nessuna sincronizzazione lo scrive.';

-- Stessa ragione del drop cascade nelle migration precedenti: aggiungere una
-- colonna a metà di una vista esistente richiede ricrearla.
